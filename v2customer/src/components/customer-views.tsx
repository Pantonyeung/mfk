...
export function CheckoutView({cart,quote,checkout,setCheckout,pending,actionState,onSubmit,onReadback,onBack,onRepair}:{
  ...
  return <section className="page checkout-page">
    ...
    <section className="checkout-payment"><SectionHeading eyebrow="付款方式" title="今次點樣付款？"/>
      ...
      {checkout.paymentMethod==='ELECTRONIC'?<div className="payment-evidence">
        <p>付款截圖只係付款證明，必須由店舖核對；上載截圖唔代表已付款確認。</p>
        <label><span>{checkout.paymentEvidence?.fileName??'選擇付款截圖'}</span><input type="file" accept="image/*" onChange={event=>{const file=event.target.files?.[0];if(!file)return;setCheckout({...checkout,paymentEvidence:{fileName:file.name,mimeType:file.type||'application/octet-stream',size:file.size,state:'LOCAL_PENDING_UPLOAD'}})}}/></label>
        {checkout.paymentEvidence?.state==='LOCAL_PENDING_UPLOAD'?<small>截圖已選擇；正式上載通道接通前唔會假裝已上載成功。</small>:null}
      </div>:null}
    </section>
