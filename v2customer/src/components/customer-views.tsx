  ...
  return <section className="hero-carousel" aria-roledescription="carousel" aria-label="磨飯品牌精選">
    <div ref={railRef} className="hero-rail" onScroll={event=>{
      ...
      {slides.map((slide,index)=><article className="hero-slide" key={slide.image} aria-label={`${index+1} / ${slides.length}`}>
        ...
        <div className="hero-message"><span>{slide.eyebrow}</span><h2>{slide.title}</h2></div>
      </article>)}
    </div>
...
function JarVisual({count}:{count:number}){
  ...
  return <div className={`memory-jar-visual level-${level}`} aria-label={count?`記憶罐有 ${count} 件餐點`:'記憶罐係空嘅'}><div className="jar-lid"/><div className="jar-glass"><i/><i/><i/><span>{count||'+'}</span></div><small>{count?count===1?'第一樣已放好':count<5?'今餐漸漸成形':'準備好去確認':'等待你揀第一樣'}</small></div>;
}
...
export function CheckoutView({cart,quote,checkout,setCheckout,pending,actionState,onSubmit,onReadback,onBack,onRepair}:{
  cart:readonly CustomerCartLine[];quote:CustomerQuoteSnapshot|null;checkout:CustomerCheckoutDraft;setCheckout:(v:CustomerCheckoutDraft)=>void;pending:CustomerPendingIntent|null;actionState:ActionState;onSubmit:()=>void;onReadback:(intent:CustomerPendingIntent)=>void;onBack:()=>void;onRepair:()=>void;
}){
  ...
  return <section className="page checkout-page">
    ...
    <section className={`safe-submit state-${actionState}`} role={unknown||waiting?'status':undefined}><span>{unknown?'結果未知':waiting?'等待中':'安全提交'}</span><h2>{unknown?'正在確認訂單結果':waiting?'等待店舖確認':quote?'準備送出落單要求':'等待店舖報價'}</h2><p>{unknown?'店舖可能已收到落單要求。請勿重複提交，先查詢原本嗰次結果。':waiting?'落單要求已送出，未有終局前唔會自動重送。':'如果結果未明，系統會保留原本嗰次落單並先讀回結果，唔會盲目重送。'}</p><StatefulAction state={actionState} labels={{default:pending?.state==='NOT_CONNECTED'?'使用原本落單再試':'確認並送出',loading:'正在安全處理',pending:'等待店舖確認',unknown:'重新確認提交結果',disabled:quote?'需要先修正變更':'等待正式報價'}} onClick={unknown&&pending?()=>onReadback(pending):onSubmit}/>{unknown?<small>呢個動作只會讀取原本結果，未有重新提交。</small>:pending?.state==='NOT_CONNECTED'?<small>本機草稿已保存，未建立正式訂單。</small>:null}</section>}
  </section>;
}
...
export function MemberView({connection,snapshot,history,pendingIntents,readingIntentId,onRefresh,onReadback,onDiscard,onFallback,onReorder,onBrowse}:{connection:CustomerConnectionState;snapshot:CustomerReadModelSnapshot|null;history:readonly CustomerHistoryProjection[];pendingIntents:readonly CustomerPendingIntent[];readingIntentId:string|null;onRefresh:()=>void;onReadback:(intent:CustomerPendingIntent)=>void;onDiscard:(id:string)=>void;onFallback:()=>void;onReorder:(order:CustomerHistoryProjection)=>void;onBrowse:()=>void;}){
  ...
  return <section className="page member-page">
    ...
    <section className="member-module seeds-module"><SectionHeading eyebrow="記憶種子" title="每次返嚟，都有段關係"/><ProjectionState state={seedsState} title="記憶種子" detail="種子數量、進度、下一個小心意同歷史必須由店舖正式會員資料提供。" onRefresh={onRefresh}/>{seedsState==='READY'?<div className="seed-dashboard"><div className="seed-value"><span>目前記憶種子</span><AnimatedValue as="strong">{member?.seeds?.valueLabel??'—'}</AnimatedValue><small>{member?.seeds?.progressLabel??'進度資料未提供'}</small></div><div className="next-care"><span>下一個小心意</span><strong>{member?.seeds?.nextBenefitLabel??'由店舖會員資料提供'}</strong><p>畫面唔會用固定種子數推算獎賞。</p></div>{member?.seeds?.history?.length?<ol className="seed-history">{member.seeds.history.map(item=><li key={`${item.occurredAt}-${item.label}`}><i/><span>{item.label}</span><time dateTime={item.occurredAt}>{new Date(item.occurredAt).toLocaleDateString('zh-HK')}</time></li>)}</ol>:null}</div>:null}</section>
