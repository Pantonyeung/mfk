...
export function CheckoutView({cart,quote,checkout,setCheckout,paymentChannels,pending,actionState,submitProbe,submitBlockReason,fallbackAvailable,onFallback,onSubmit,onReadback,onBack,onRepair,onPaymentEvidence}:{
  cart:readonly CustomerCartLine[];quote:CustomerQuoteSnapshot|null;checkout:CustomerCheckoutDraft;setCheckout:(v:CustomerCheckoutDraft)=>void;paymentChannels:NonNullable<CustomerReadModelSnapshot['paymentChannels']>;pending:CustomerPendingIntent|null;actionState:ActionState;submitProbe:Readonly<{attempt:number;total:number}>|null;submitBlockReason:string|null;fallbackAvailable:boolean;onFallback:()=>void;onSubmit:()=>void;onReadback:(intent:CustomerPendingIntent)=>void;onBack:()=>void;onRepair:()=>void;onPaymentEvidence:(file:File)=>void;
}){
  const unknown=pending?.state==='UNKNOWN';
  const waiting=pending?.state==='PENDING';
  const processing=actionState==='loading';
  const fallback=pending?.state==='NOT_CONNECTED'&&fallbackAvailable;
  ...
  return <section className="page checkout-page">
    ...
    <section className="checkout-payment"><SectionHeading eyebrow="付款方式" title="今次點樣付款？"/>
      ...
      {checkout.paymentMethod==='ELECTRONIC'?<div className="electronic-payment-flow">
        ...
        <div className="payment-evidence"><p>完成付款後，再上傳付款截圖畀店舖核對；提供截圖唔代表付款已確認。</p><label className="evidence-picker"><span>{checkout.paymentEvidence?.fileName??'選擇付款截圖'}</span><input key={checkout.paymentEvidence?checkout.paymentEvidence.fileName+'-'+checkout.paymentEvidence.state:'empty'} type="file" accept="image/*" onChange={event=>{const file=event.target.files?.[0];if(!file)return;onPaymentEvidence(file)}}/></label>{checkout.paymentEvidence?<small>已選擇：{checkout.paymentEvidence.fileName}</small>:null}</div>
      </div>:null}
    </section>
