...
export function CheckoutView({cart,quote,checkout,setCheckout,pending,actionState,onSubmit,onReadback,onBack,onRepair}:{
  ...
  return <section className="page checkout-page">
    ...
    <section className="checkout-payment"><SectionHeading eyebrow="付款方式" title="今次點樣付款？"/>
      ...
      {checkout.paymentMethod==='ELECTRONIC'?<div className="payment-evidence"><p>付款截圖會交由店舖核對；提供截圖唔代表付款已確認。</p><label className="evidence-picker"><span>{checkout.paymentEvidence?.fileName??'選擇付款截圖'}</span><input type="file" accept="image/*" onChange={event=>{const file=event.target.files?.[0];if(!file)return;setCheckout({...checkout,paymentEvidence:{fileName:file.name,mimeType:file.type||'application/octet-stream',size:file.size,state:'LOCAL_PENDING_UPLOAD'}})}}/></label>{checkout.paymentEvidence?<small>已選擇：{checkout.paymentEvidence.fileName}</small>:null}</div>:null}
    </section>
