...
describe('Owner dining cart-entry mindset R4',()=>{
  it('opens ordinary hold first when every cart line is takeaway',()=>{
    expect(initialHoldModeForLines([
      {id:'a',productId:'a',name:'A',qty:1,unitMinor:1000,serviceMode:'takeaway'},
      {id:'b',productId:'b',name:'B',qty:1,unitMinor:1200,serviceMode:'takeaway'},
    ])).toBe('cart');
  });
  ...
  it('opens dining first when any cart line is dine-in, including mixed carts',()=>{
    expect(initialHoldModeForLines([
      {id:'a',productId:'a',name:'A',qty:1,unitMinor:1000,serviceMode:'takeaway'},
      {id:'b',productId:'b',name:'B',qty:1,unitMinor:1200,serviceMode:'dine-in'},
    ])).toBe('dining');
  });
  ...
  it('passes the cart-derived default into the existing hold workspace without creating another dining engine',()=>{
