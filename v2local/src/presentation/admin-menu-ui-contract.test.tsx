...
import {ProductCardText,splitProductName} from '../features/ordering/OrderingWorkspace.tsx';
...
describe('Admin Menu physical acceptance UI contracts',()=>{
  it('renders the POS product card as full text with name and price',()=>{
    ...
    expect(html).toContain('>古早味紫米飯</span>');
    expect(html).toContain('>糰</span>');
    expect(html).toContain('>$41.00</strong>');
    expect(html).not.toContain('<img');
  });

  it('wraps the seventh product-name character onto a new line',()=>{
    expect(splitProductName('1234567')).toEqual(['123456','7']);
    expect(splitProductName('古早味紫米飯糰')).toEqual(['古早味紫米飯','糰']);
  });
