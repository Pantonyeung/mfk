...
describe('SMT A3b Required Fast Lane',()=>{
  ...
  it('blocks checkout until Required tasks are resolved and wires the dedicated 必選區',()=>{
    ...
    expect(app).toContain("{id:'required',label:'必選區',count:requiredWork.length}");
    expect(app).toContain("<RequiredFastLaneWorkspace cart={cart} products={workspaceProducts} onApply={applyRequired}/>");
