...
export function priceCustomerCart(
  ...
  for(const line of cart){
    ...
    if(line.note?.trim())detailParts.push('備註：'+line.note.trim());
...
function activeCatalog(){
  ...
  return {envelope,catalog:projectSyncedOrderingCatalog('takeaway',envelope)};
}
    ...
    try{
      ...
      if(existing){
        await postJson('/api/customer/smt/orders/ack',{
          ...
          state:'CONFIRMED',
          canonicalOrderId:existing.id,
          canonicalDisplay:existing.display,
