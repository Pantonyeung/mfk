...
describe('SMM web acceptance intake',()=>{
  ...
  it('feeds isolated acceptance intent into the canonical SMM ingress and ACKs once',async()=>{
    ...
    const ingress={
      readSnapshot:vi.fn(()=>({orders:[],dineSessions:[],work:[]})),
      submit:vi.fn(()=>({
        protocolVersion:1 as const,
    ...
    vi.stubGlobal('fetch',vi.fn(async(input:RequestInfo|URL,init?:RequestInit)=>{
      ...
      if(url.includes('/projection')){
        return new Response(JSON.stringify({state:'PROJECTED'}),{
          status:200,headers:{'content-type':'application/json'},
        });
      }
      if(url.includes('/ack')){
    ...
    expect(calls.some(call=>call.url==='/__mfk/smm-acceptance/projection')).toBe(true);
  });
  ...
  it('retries the same ACK identity after a transient public failure without resubmitting the order',async()=>{
    ...
    const ingress={readSnapshot:vi.fn(()=>({orders:[],dineSessions:[],work:[]})),submit:vi.fn(()=>({
    ...
    vi.stubGlobal('fetch',vi.fn(async(input:RequestInfo|URL,init?:RequestInit)=>{
      ...
      if(url.includes('/projection'))return new Response(JSON.stringify({state:'PROJECTED'}),{status:200,headers:{'content-type':'application/json'}});
      if(url.includes('/ack')){
        ackAttempts++;
