  ...
  return <main className="checkout-workspace" aria-label="結帳">
    <aside className="checkout-order-panel" aria-label="訂單明細">
      <header>
        ...
        <div><small>ORDER</small><strong>#{view.order.orderId}</strong></div>
      </header>
    ...
    <section className="checkout-payment-stage">
      {view.channels.length?<header className="checkout-channel-row" aria-label="訂單渠道">
        {view.channels.map(channel=><button type="button" key={channel.id} aria-pressed={channel.selected} className={channel.selected?'active':''} onClick={()=>actions.onSelectChannel(channel.id)}><b>{channel.label}</b>{channel.helperLabel?<small>{channel.helperLabel}</small>:null}</button>)}
      </header>:null}

      <div className="checkout-payment-grid">
        <section className="checkout-method-panel" aria-label="付款方式">
          <div className="checkout-section-title"><small>PAYMENT METHOD</small><h1>收款</h1></div>
          ...
          {view.validationMessage?<div className="checkout-payment-alert" role="status"><b>現金不足</b><span>{view.validationMessage}</span></div>:null}
        ...
        <section className="checkout-cash-panel" aria-label="實收輸入">
          <div className="checkout-cash-display"><small>實收金額</small><strong>{view.cashInput||'0'}</strong></div>
          <div className="checkout-quick-cash">
            ...
            {['50','100','200','500'].map(amount=><button type="button" key={amount} disabled={processing||success} onClick={()=>actions.onCashKey(amount)}>+{amount}</button>)}
          </div>
