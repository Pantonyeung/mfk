...
export function ComboWorkspace({
  ...
  const selectedPools=combo?[combo.mainPoolId,...combo.addonPoolIds].filter(Boolean).map(id=>poolById.get(id!)).filter((pool):pool is SyncedComboPool=>Boolean(pool)):[];
  ...
  return <div className="combo-workspace">
    <header className="combo-title"><div><h2>套餐</h2><p>套餐、Pool、價差同可選商品全部來自 Admin 已發布版本。</p></div><strong>{combo.name}　{money(total)}</strong></header>
