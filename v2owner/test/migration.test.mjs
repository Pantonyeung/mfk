...
test('Stage01 top bar uses Owner-approved canonical logo asset only',()=>{
  const app=fs.readFileSync(path.join(srcRoot,'App.tsx'),'utf8');
  const css=fs.readFileSync(path.join(srcRoot,'styles.css'),'utf8');
  const assetDir=path.join(srcRoot,'assets/brand');
  const assetModule=fs.readFileSync(path.join(assetDir,'canonical-logo.ts'),'utf8');
  const proof=fs.readFileSync(path.join(assetDir,'ASSET_PROOF.txt'),'utf8');

  assert.doesNotMatch(app,/className="brand-mark">磨</);
  assert.match(app,/OWNER_CANONICAL_LOGO_SRC/);
  assert.match(app,/alt="磨飯 More Fun"/);
  assert.match(app,/data-asset-id=\{OWNER_CANONICAL_LOGO_ASSET_ID\}/);
  ...
  for(const file of[
    'canonical-logo.ts',
    'canonical-logo.part1.b64',
    'canonical-logo.part2.b64',
    'canonical-logo.part3.b64',
    'canonical-logo.part4.b64',
    'ASSET_PROOF.txt',
  ])assert.equal(fs.existsSync(path.join(assetDir,file)),true,file);

  assert.match(assetModule,/MFK-BRAND-LOGO-OWNER-CANONICAL-001/);
  assert.match(assetModule,/data:image\/webp;base64/);
  assert.match(proof,/LOGO_SOURCE = OWNER_APPROVED_CANONICAL_LOGO/);
  assert.match(proof,/METHOD = CANONICAL_ASSET_DIRECT_USE/);
  assert.match(proof,/DIRECT_CROP = NO/);
  assert.match(proof,/REDRAWN_LOGO = NO/);
  assert.match(proof,/STOCK_ASSET = NO/);
  assert.match(proof,/THIRD_PARTY_ICON = NO/);

  // Normal Today/Home remains management-first; no mascot/IP hero added.
  assert.doesNotMatch(app,/mascot|ip-hero|character-hero|boy-ip|girl-ip/i);
});
