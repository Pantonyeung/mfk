...
import {filterCustomerOwnedOrders,projectCustomerPublicSnapshot} from './customer-public-projection.ts';
...
export class AdminSyncStore{
  ...
  async fetch(request){
    ...
    if(url.pathname==='/customer-doorbell'&&request.method==='POST'){
      ...
      if(type!=='CUSTOMER_QUOTE_AVAILABLE'&&type!=='CUSTOMER_ORDER_AVAILABLE'){
    ...
    if(url.pathname==='/customer-orders'&&request.method==='GET'){
      const submissionIds=url.searchParams.getAll('submissionId').map(value=>String(value).trim()).filter(Boolean).slice(0,24);
      return json({orders:filterCustomerOwnedOrders(await this.projectionOrders(),submissionIds)});
    }
...
export default {
  async fetch(request,env){
    ...
    if(url.pathname.startsWith('/api/customer/')){
      ...
      const customer=env.CUSTOMER_RUNTIME.get(env.CUSTOMER_RUNTIME.idFromName(storeId));
      const admin=env.ADMIN_SYNC.get(env.ADMIN_SYNC.idFromName(storeId));
      ...
      if(url.pathname==='/api/customer/snapshot'){
        ...
        const orderUrl=new URL('https://internal/customer-orders');
        for(const submissionId of url.searchParams.getAll('submissionId').slice(0,24)){
          if(String(submissionId).trim())orderUrl.searchParams.append('submissionId',String(submissionId).trim());
        }
        const orderResponse=await admin.fetch(new Request(orderUrl.toString(),{method:'GET'}));
        const orderBody=orderResponse.ok?await orderResponse.json():{orders:[]};
        return json(
          projectCustomerPublicSnapshot(active,Array.isArray(orderBody.orders)?orderBody.orders:[]),
          200,
          cors(request)
        );
      }
      ...
      if(targetPath&&(url.pathname==='/api/customer/quote'||url.pathname==='/api/customer/orders/submit')){
        ...
        if(!activeResponse.ok)return json({code:'CUSTOMER_CONFIG_NOT_PUBLISHED'},503,cors(request));
        const active=await activeResponse.json();
        const policy=active?.snapshot?.customerChannelPolicy;
        if(policy&&policy.enabled===false)return json({code:'CUSTOMER_CHANNEL_DISABLED'},503,cors(request));
      }
      if(targetPath){
        const target=new URL(request.url);
