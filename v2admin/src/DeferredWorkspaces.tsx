...
function Header({title,description,badge='P1 保留能力'}:{title:string;description:string;badge?:string}){
...
export function InventoryWorkspace(){
  ...
  return <section className="admin-editor-page">
    ...
    {filtered.length===0?<div className="admin-read-empty">未有庫存統計項目。</div>:<section className="admin-read-table"><header><span>品項</span><span>目前數量</span><span>單位</span><span>提示值</span><span>Revision</span></header>{filtered.map(row=><article key={row.id}><span>{row.displayName}</span><span>{row.quantity}</span><span>{row.baseUnit}</span><span>{row.lowStockThreshold||'—'}</span><span>R{row.revision}</span></article>)}</section>}
  </section>;
}
...
export function PresentationWorkspace({surface}:{surface:'CUSTOMER'|'OWNER'|'FRONTLINE'}){
  ...
  return <section className="admin-editor-page">
    <Header title={title} description="只管理 bounded presentation 設定：顯示內容、區塊、欄數同快捷商品。商品名、價格、供應、訂單仍由正式資料 authority 決定。" badge="顯示設定"/>
    <div className="admin-policy-grid two">
...
export function StoreBindingWorkspace(){
  ...
  return <section className="admin-editor-page">
    <Header title="門店授權映射" description="管理平台門店身份同磨飯 Store identity 關係。呢度建立正式對應設定，但未連 Provider 前唔會假裝平台已接受。" badge="身份映射設定"/>
...
export function Customer360Workspace(){
  ...
  return <section className="admin-editor-page">
    <Header title="顧客資料" description="用電話／顧客編號查看正式 Customer snapshot、消費摘要、會員積分同 CRM 標籤。電話唔等於 customerId，Loyalty 亦唔等於 Consent。" badge="CRM Lite"/>
    ...
    {!query?<div className="admin-read-empty">輸入電話或顧客編號開始查詢。</div>:!found?<div className="admin-read-empty">目前正式 Customer read model 入面搵唔到呢位顧客。</div>:<div className="admin-policy-grid two"><section className="admin-read-card"><header><h2>{found.displayName||found.phone}</h2><span>{found.relationshipStatus||'未分類'}</span></header><p>顧客編號：{found.id}</p><p>電話：{found.phone}</p><p>標籤：{found.tags?.join('、')||'—'}</p></section><section className="admin-read-card"><header><h2>消費／會員</h2><span>{new Date(found.updatedAt).toLocaleString('zh-HK')}</span></header><p>訂單：{found.orders}</p><p>累計消費：{'HK$'+(found.spendMinor/100).toFixed(2)}</p><p>積分：{found.points??'—'}</p></section></div>}
  </section>;
}
...
export function CouponsWorkspace(){
  ...
  return <section className="admin-editor-page">
    <header className="admin-editor-head"><div><small>優惠政策</small><h1>優惠券</h1><p>管理優惠券名稱、Code、固定金額／百分比、有效期同啟用狀態；真正 Quote 由唯一 Pricing authority 執行。</p></div><div className="admin-editor-actions"><button onClick={add}>新增優惠券</button></div></header>
...
export function RfmWorkspace(){
  ...
  const latest=rows[0];
  return <section className="admin-editor-page">
    <Header title="客戶分群分析" description="只讀正式 Reporting projection；RFM 分群唔會自動寫入 CRM tag。" badge="CRM Lite 報表"/>
    ...
    {!latest?<div className="admin-read-empty">目前未有正式 RFM projection。</div>:null}
  </section>;
}
...
export function AnnouncementsWorkspace(){
  ...
  return <section className="admin-editor-page">
    <header className="admin-editor-head"><div><small>公告設定</small><h1>公告／通知</h1><p>管理公告內容、對象同啟用狀態。通知發送通道屬後續 connection，呢度唔會將「保存公告」假裝成「已送達」。</p></div><div className="admin-editor-actions"><button onClick={add}>新增公告</button></div></header>
