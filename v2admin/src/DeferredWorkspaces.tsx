...
export function StoreBindingWorkspace(){
  ...
  return <section className="admin-editor-page">
    ...
    <div className="admin-policy-grid two">
      <article className="admin-policy-card"><h2>對應草稿</h2><label><span>平台</span><input value={provider} onChange={event=>setProvider(event.target.value)} placeholder="Provider"/></label><label><span>平台門店 ID</span><input value={externalStore} onChange={event=>setExternalStore(event.target.value)} placeholder="平台門店"/></label><label><span>磨飯門店 ID</span><input value={mfkStore} onChange={event=>setMfkStore(event.target.value)} placeholder="磨飯門店"/></label></article>
...
export function Customer360Workspace(){
  ...
  return <section className="admin-editor-page">
    ...
    <div className="admin-policy-grid two"><section className="admin-read-card"><header><h2>顧客資料</h2><span>尚未啟用</span></header><div className="admin-read-empty">CUSTOMER_PROFILE_尚未啟用</div></section><section className="admin-read-card"><header><h2>訂單</h2><span>尚未啟用</span></header><div className="admin-read-empty">CUSTOMER_ORDER_HISTORY_尚未啟用</div></section></div>
  </section>;
}
...
export function AdvancedWorkspace(){
  ...
  return <section className="admin-editor-page">
    ...
    <div className="admin-policy-grid">
      ...
      <article className="admin-policy-card"><h2>系統狀態</h2><label className="admin-toggle"><input type="checkbox" checked={diagnostics} onChange={event=>set系統狀態(event.target.checked)}/><span>顯示診斷工具</span></label><span className="admin-not-wired-chip">稍後開放</span></article>
