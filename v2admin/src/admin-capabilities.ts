...
export const ADMIN_CAPABILITY_GROUPS:readonly AdminCapabilityGroup[]=[
  ...
  {id:'menu',label:'菜單',capabilities:[
    ...
    c('categories','分類與結構','/admin/catalog/categories','READY','ADMIN_CONFIG','建立、排序及管理商品分類。'),
    c('menu-sort','顯示排序','/admin/catalog/menu-display','READY','ADMIN_CONFIG','管理菜單顯示次序同可見性規則。'),
  ...
  {id:'operations',label:'營運',capabilities:[
    ...
    c('print-templates','打印模板','/admin/print/templates','READY','PRINT_LOGICAL','管理打印模板同輸出規則。'),
    c('print-rules','打印規則','/admin/print/rules','READY','PRINT_LOGICAL','管理商品輸出、堂食打印同打印分流規則。'),
    c('device-health','裝置管理','/admin/devices','GOVERNANCE','GOVERNANCE','顯示裝置類型、信任狀態、版本同設定差異。'),
    c('ota','裝置版本','/admin/ota','GOVERNANCE','GOVERNANCE','管理已批准版本、裝置目前版本同還原記錄。'),
    c('announcement','公告／通知','/admin/store/announcements','P1','ADMIN_CONFIG','保留公告／通知配置。'),
  ]},
  {id:'channels',label:'渠道',capabilities:[
    c('channel-overview','平台管理','/admin/channels','READY','CHANNEL','平台狀態、規則同設定入口。'),
    c('accept-policy','接單規則','/admin/channels/accept-policy','READY','CHANNEL','管理接單規則草稿。'),
    c('sync-policy','供應同步','/admin/channels/sync-policy','READY','CHANNEL','管理平台供應同步規則草稿。'),
  ...
  {id:'settings',label:'設定',capabilities:[
    ...
    c('advanced','生效設定','/admin/system/advanced','GOVERNANCE','ADMIN_CONFIG','顯示目前生效值、來源、覆寫同安全底線。'),
  ]},
];
