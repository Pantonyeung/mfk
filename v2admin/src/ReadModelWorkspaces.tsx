...
export function OverviewWorkspace(){
  const readiness=[
    ['菜單／設定','未確認','/admin/publish'],
    ['平台','未確認','/admin/channels'],
    ['裝置／打印機','未確認','/admin/devices'],
    ['營業日記錄','未確認','/admin/business-day'],
  ] as const;
  return <section className="admin-editor-page">
    ...
    <div className="admin-overview-columns">
      <section className="admin-read-card">
        ...
        <small>營運準備只係狀態提示；異常或未確認狀態唔會自動阻止交易。</small>
      </section>
      ...
      <section className="admin-read-card"><header><h2>待處理事項</h2><span>尚未啟用</span></header><div className="admin-read-empty">待處理事項資料尚未啟用。</div><Link to="/admin/action-queue">查看待處理事項</Link></section>
    </div>
  </section>;
}
export function CapacityWorkspace(){
  ...
  return <section className="admin-editor-page">
    <header className="admin-editor-head"><div><small>產能設定尚未啟用</small><h1>每日產能／原料額度</h1><p>設定提示同產能規則；預設唔會無聲阻止交易。</p></div><div className="admin-editor-actions"><button className="publish" disabled>尚未可發布</button></div></header>
...
export function OpenOrdersWorkspace(){
  ...
  return <section className="admin-editor-page">
    ...
    <section className="admin-read-table"><header><span>訂單</span><span>來源</span><span>金額</span><span>狀態</span><span>時間</span></header><div className="admin-read-empty">訂單資料尚未啟用</div></section>
  </section>;
}
...
export function OrdersHistoryWorkspace(){
  ...
  return <section className="admin-editor-page">
    ...
    <section className="admin-read-table"><header><span>訂單</span><span>來源</span><span>金額</span><span>狀態</span><span>完成時間</span></header><div className="admin-read-empty">訂單記錄尚未啟用</div></section>
  </section>;
}
...
export function SalesReportWorkspace(){
  ...
  return <section className="admin-editor-page">
    ...
    <section className="admin-read-table"><header><span>日期</span><span>總額</span><span>調整</span><span>淨額</span><span>訂單</span></header><div className="admin-read-empty">報表資料尚未啟用</div></section>
  </section>;
}
...
export function OperationsReportWorkspace(){
  return <section className="admin-editor-page">
    ...
    <section className="admin-read-card"><header><h2>營運時間線</h2><span>尚未啟用</span></header><div className="admin-read-empty">營運報表尚未啟用</div></section>
  </section>;
}
...
export function AuditWorkspace(){
  ...
  return <section className="admin-editor-page">
    ...
    <section className="admin-read-table"><header><span>時間</span><span>操作人</span><span>範圍</span><span>操作</span><span>結果</span></header><div className="admin-read-empty">操作記錄尚未啟用</div></section>
  </section>;
}
